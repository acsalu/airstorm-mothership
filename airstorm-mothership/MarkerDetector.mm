...
MarkerDetector::MarkerDetector(CameraCalibration calibration, ASMarkerDetectionViewController *viewController)
...
void MarkerDetector::recognizeMarkers(const cv::Mat& grayscale, std::vector<Marker>& detectedMarkers)
{
    ...
    if (goodMarkers.size() > 0)
    {
        ...
        {  
            ...
            [_viewController detectMarkerWithId:marker.id];
            for (int c = 0; c <4; c++)
            {
